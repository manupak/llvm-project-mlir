...
struct BlockwiseReduceRewritePattern
  ...
  ArrayAttr appendReducedDimView(Location loc, 
    ...
    BottomUpTMBuilder threadsToTensor(rewriter, toReduceShape, loc);
    for (uint32_t i = 0; i < toReduceShape.size(); ++i) {
      if(i == axis){
        threadsToTensor.broadcast({i}, {toReduceShape[i]});
      }
      else{
        threadsToTensor.passThrough({i}, {i});
      }
    }
    SmallVector<Attribute> regToInputTensorViewTrs {regToInputTensorView.getOps()};
    for(Attribute trMap : threadsToTensor.get().getOps()){
  ...
  LogicalResult matchAndRewrite(BlockwiseReduceOp op,
    ...
    TransformAttr upperTr = inputView.getOps().back();
    ...
    //Following RAII scope will create reduction loops.
    {
      ...
      if(blockSize < nonReductionDimSizeProduct){
        ...
        // number of threads.
        {
          ...
          Value accReg = rewriter.create<GpuAllocOp>(loc, loadTypeInputReg);
          arith::ConstantOp initVal = getReductionInitValue(op, rewriter);
          rewriter.create<FillOp>(loc, accReg, initVal.getResult());
        ...
        // LDS
        {
            ...
            ArrayRef<int64_t>(strides), /*forceUnroll=*/true, /*useIndexDiffs=*/true);
          {
            ...
            Value loadVal = rewriter.create<InBoundsLoadOp>(loc, accReg.getType(), accReg, zeroConstantOp);
            rewriter.create<InBoundsStoreOp>(loc, loadVal, workspaceLDSBuffer, LDSStoreCoords);
          }
        }
...
LogicalResult ThreadwiseReadIntoRewritePattern::matchAndRewrite(
  ...
  auto [srcBuffer, srcTransforms] = untransform(b, sourceView, op.getExtraSrcViews().value_or(nullptr));
  MemRefType srcBufferType = srcBuffer.getType().cast<MemRefType>();
  auto [dstBuffer, dstTransforms] = untransform(b, destView, op.getExtraDstViews().value_or(nullptr));
  ...
  if(srcBufferType.getMemorySpace()){
    srcAddrSpace = srcBufferType.getMemorySpace().cast<gpu::AddressSpaceAttr>().getValue();
  }
  // Unless specified it is assumed to be private
  gpu::AddressSpace dstAddrSpace = gpu::AddressSpace::Private;
  if(dstBufferType.getMemorySpace()){
    dstAddrSpace = dstBufferType.getMemorySpace().cast<gpu::AddressSpaceAttr>().getValue();
  }
  ...
  // if src addr space is registers and with no transforms, there is no constrain on vectorization coming from src
  const int64_t maxVectorLenBits = 128;
  int64_t srcVectorLen = std::min(maxVectorLenBits, srcBufferType.getNumElements());
  ...
  // if dest addr space is registers and with no transforms, there is no constrain on vectorization coming from dest
  int64_t destVectorLen = std::min(maxVectorLenBits, dstBufferType.getNumElements());
      ...
      forceUnroll, useIndexDiffs);
  {
    ...
    if(srcAddrSpace == gpu::AddressSpace::Private){
      ...
      loaded = b.create<GlobalLoadOp>(
          loc, loadType, srcBuffer, loadLoop.getValidity(/*domain=*/0),
