...
LogicalResult ThreadwiseReadIntoOp::verify() {
  ...
  while (auto transform = dyn_cast_or_null<TransformOp>(destBuffer.getDefiningOp())) {
  ...
  gpu::AddressSpace destGpuMemSpace = gpu::AddressSpace::Private;
  if(destMemSpaceAttr){
    destGpuMemSpace = destMemSpaceAttr.cast<gpu::AddressSpaceAttr>().getValue();
  }

  TypedValue<ShapedType> srcBuffer = getSource();
  while (auto transform = dyn_cast_or_null<TransformOp>(srcBuffer.getDefiningOp())) {
  ...
  SmallVector<std::pair<gpu::AddressSpace, gpu::AddressSpace>, 2> allowedCombinations = {
    {gpu::AddressSpace::Global, gpu::AddressSpace::Private},
    {gpu::AddressSpace::Private, gpu::AddressSpace::Workgroup},
  };

  if(!llvm::any_of(allowedCombinations, 
      [&](std::pair<gpu::AddressSpace, gpu::AddressSpace> allowedComb){
        return allowedComb.first == srcGpuMemSpace && allowedComb.second == destGpuMemSpace;
      }
    )){
  ...
  if(srcGpuMemSpace == gpu::AddressSpace::Global){
    ...
    if(maybeExtraSrcViews && !maybeExtraSrcViews.value().empty()){
      inputShape = maybeExtraSrcViews.value()[0].cast<TransformMapAttr>().getUpperBounds();
    }
    else{
    ...
    if (inputShape.size() != 3)
      return emitOpError("global source view must accept (bid, tid, iter) coordinates");
  }
  else{
    ...
    if(!maybeExtraSrcViews || maybeExtraSrcViews.value().empty()){
      ...
      if(getSource().getType().getRank() != 3){
        return emitOpError("register source with view must accept (bid, tid, iter) coordinates.");
      }
    }
  }
  ...
  if(destGpuMemSpace == gpu::AddressSpace::Workgroup){
    ...
    if(maybeExtraDstViews && !maybeExtraDstViews.value().empty()){
      outputShape = maybeExtraDstViews.value()[0].cast<TransformMapAttr>().getUpperBounds();
    }
    else{
    ...
    if (outputShape.size() != 3)
      return emitOpError("workgroup dest view must accept (bid, tid, iter) coordinates");
  }
  else{
    // This should be private
    if(!maybeExtraDstViews || maybeExtraDstViews.value().empty()){
      ...
      if(getDest().getType().getRank() != 3){
        return emitOpError("register dest with view must accept (bid, tid, iter) coordinates.");
      }
    }
  }
