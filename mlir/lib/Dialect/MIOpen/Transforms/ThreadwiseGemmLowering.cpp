  ...
  LogicalResult matchAndRewrite(XdlopsGemmV2Op op,
    ...
    for (int64_t i = 0; i < vectorNumber; ++i) {
      ...
      auto mfma = innerLoopb.create<amdgpu::MFMAOp>(
          loc, vectorType, /*m=*/mfmaNonKDim, /*n=*/mfmaNonKDim, /*k=*/xcs.k,
          xcs.blocksMfma, argA, argB, vectorC,
          /*cbsz=*/imms[i].cbsz, /*abid=*/imms[i].abid, /*blgp=*/imms[i].blgp,
          /*reducePrecision=*/false, /*negateA=*/false,
          /*negateB=*/false, /*negateC=*/false);
      Value vectorD = mfma.destD();
